...
def save_data(tgt_folder, src_param, ds, id_area, decimal):
    print('--> save data')
...
def calculate_stats_w_mean(df, area_threshold, processor):   
    ...
    if processor == 'cpu':
        print('+++++++++++ CPU +++++++++++')
        # (1) remove data rows where all monthes are NULL
        #print(df.loc[df.id_area == 3])
        indexNames = df[df.dec.isnull() & df.nov.isnull() & df.oct.isnull() & df.sep.isnull() & df.aug.isnull() & df.jul.isnull() & df.jun.isnull() & df.may.isnull() & df.apr.isnull() & df.mar.isnull() & df.feb.isnull() & df.jan.isnull()].index
        #print(indexNames)
        print(df.loc[df.id_area == test_id])
        df = df.drop(indexNames, inplace=False)
        print(df.shape)
        #sys.exit()
        # (2) create an numpy array
        ...
        # (5.1) delete rows where percent_area is below threshold
        #todo
        # (6) summarizing weighted data results as average monthly data
        avg_mean = []
    ...
    # replace values by -99999 where area == 0
    """
    df_w_mean['jan'] = np.where(df_w_mean['area'] == 0, -9999900.0, df_w_mean['jan'])
    df_w_mean['feb'] = np.where(df_w_mean['area'] == 0, -9999900.0, df_w_mean['feb'])
    df_w_mean['mar'] = np.where(df_w_mean['area'] == 0, -9999900.0, df_w_mean['mar'])
    df_w_mean['apr'] = np.where(df_w_mean['area'] == 0, -9999900.0, df_w_mean['apr'])
    df_w_mean['may'] = np.where(df_w_mean['area'] == 0, -9999900.0, df_w_mean['may'])
    df_w_mean['jun'] = np.where(df_w_mean['area'] == 0, -9999900.0, df_w_mean['jun'])
    df_w_mean['jul'] = np.where(df_w_mean['area'] == 0, -9999900.0, df_w_mean['jul'])
    df_w_mean['aug'] = np.where(df_w_mean['area'] == 0, -9999900.0, df_w_mean['aug'])
    df_w_mean['sep'] = np.where(df_w_mean['area'] == 0, -9999900.0, df_w_mean['sep'])
    df_w_mean['oct'] = np.where(df_w_mean['area'] == 0, -9999900.0, df_w_mean['oct'])
    df_w_mean['nov'] = np.where(df_w_mean['area'] == 0, -9999900.0, df_w_mean['nov'])
    df_w_mean['dec'] = np.where(df_w_mean['area'] == 0, -9999900.0, df_w_mean['dec'])
    """
    # drop rows where area == 0
    dropRows = df_w_mean.loc[df_w_mean.area == 0].index
    df_w_mean = df_w_mean.drop(dropRows, inplace=False)
    #print(df_w_mean.loc[df_w_mean.area == 0])
    #sys.exit()
    return df_w_mean
    ...
    for src_param in files:
        ...
        for area in areaContainer:
            ...
            if df.empty:
                ...
                #sys.exit()
            # save dataset
            save_data(tgt_folder, src_param, df, id_area, decimal)
        e0 = time.time()
