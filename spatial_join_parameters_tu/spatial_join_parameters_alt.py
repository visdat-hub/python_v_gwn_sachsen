...
def get_arguments():
    ...
    src_hydrotop, src_param, tgt_file = None, None, None
    i = 1
    ...
    if src_hydrotop == None or src_param == None or tgt_file == None :
        print("ERROR: arguments of program call missing")
        sys.exit()

    return src_hydrotop, src_param, tgt_file
...
def read_areas(area_folder):
    ...
    for f in files:
        ...
        if df_intersect.size == 0:
            sys.exit('ERROR in read_areas(): ' + f)
...
def calculate_stats_w_mean_alt(df):
    ...
    df_stat['jan'] = (df_stat['jan']['sum'] / df_stat['area']['sum']).round(0).astype(int)
    df_stat['feb'] = (df_stat['feb']['sum'] / df_stat['area']['sum']).round(0).astype(int)
    df_stat['mar'] = (df_stat['mar']['sum'] / df_stat['area']['sum']).round(0).astype(int)
    df_stat['apr'] = (df_stat['apr']['sum'] / df_stat['area']['sum']).round(0).astype(int)
    df_stat['may'] = (df_stat['may']['sum'] / df_stat['area']['sum']).round(0).astype(int)
    df_stat['jun'] = (df_stat['jun']['sum'] / df_stat['area']['sum']).round(0).astype(int)
    df_stat['jul'] = (df_stat['jul']['sum'] / df_stat['area']['sum']).round(0).astype(int)
    df_stat['aug'] = (df_stat['aug']['sum'] / df_stat['area']['sum']).round(0).astype(int)
    df_stat['sep'] = (df_stat['sep']['sum'] / df_stat['area']['sum']).round(0).astype(int)
    df_stat['oct'] = (df_stat['oct']['sum'] / df_stat['area']['sum']).round(0).astype(int)
    df_stat['nov'] = (df_stat['nov']['sum'] / df_stat['area']['sum']).round(0).astype(int)
    df_stat['dec'] = (df_stat['dec']['sum'] / df_stat['area']['sum']).round(0).astype(int)
    ...
    df_w_mean = pd.DataFrame({
        ...
        'area': df_stat['area']['sum'].astype(int),
        'jan': df_stat['jan']['sum'],
        'feb': df_stat['feb']['sum'],
        'mar': df_stat['mar']['sum'],
        'apr': df_stat['apr']['sum'],
...
def save_data(tgt_folder, src_param, ds, id_area):
    ...
    if '_' in fn_param:
        ...
        year = fn_param
        parameter = os.path.split(src_param)[0].split('/')[-1]
        scenario = os.path.split(src_param)[0].split('/')[-2]
...
if __name__ ==  '__main__':
    ...
    area_folder, src_folder, tgt_folder = get_arguments()
    ...
    i = 0
    l = len(files)
    for src_param in sorted(files):
        i = i + 1
        print('--> current file : ' + src_param, i)
        print('--> file ', i, ' von ', l)
        # read parameter
        df_param = read_parameter( src_param )
        ...
        for area in areaContainer:
            ...
            df = calculate_stats_w_mean(df, 0)
            # save statistic data
