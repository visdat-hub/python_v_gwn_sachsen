...
def read_areas(area_folder):
    ...
    for f in files:
        ...
        #print(f_intersect)
        df_intersect = pd.DataFrame({'id_hydrotop' : f_intersect['idhydrotope_org'][:].astype('int64'), 'id_area' : f_intersect['idarea_data'][:], 'area' : f_intersect['area'][:]})
...
def read_parameter(src_param):
    ...
    if df_param.shape[1] == 13 :
        ...
        df_param['index'] = df_param['index'].astype('int64')
    ...
    df_param[['jan','feb','mar','apr','may','jun','jul','aug','sep','oct','nov','dec']] = df_param[['jan','feb','mar','apr','may','jun','jul','aug','sep','oct','nov','dec']].replace(-19998, np.nan)
...
def do_spatial_join(df_intersect, df_param, df_areasize):
    ...
    print('--> df_join.shape)[0], ', df_join.shape[0])
    print('--> df_intersect.shape)[0], ', df_intersect.shape[0])
...
if __name__ ==  '__main__':
    ...
    for src_param in sorted(files):
        ...
        for area in areaContainer:
            ...
            #print('--> df_hydrotop: ', df_hydrotop)
            #print('--> df_param: ', df_param)
            #print('--> df_areasize[id_area]: ', df_areasize[id_area])
